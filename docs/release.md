...
流程：构建 Word diff runtime → 导入临时签名钥匙串 → Developer ID Archive → 使用 Homebrew 安装的 `create-dmg` 创建并签名 DMG → 公证/staple DMG → 挂载并验证 DMG 与其中的应用 → GitHub Release。Debug 测试和无签名 Release 构建由 tag 所在提交此前的分支/PR CI 负责，Release 工作流不重复执行测试；单独推送 tag 不会再次触发 CI。公证服务会同时检查最外层 DMG 和其中的应用，工作流只提交最外层 DMG，并将票据 staple 到最终发布的 DMG。使用 Xcode 默认 DerivedData；Archive 和临时文件在 runner 临时目录中。公证非 Accepted 时不会发布。
...
应用版本使用 tag 的数字部分，构建号使用 GitHub run number。唯一的发布资产是 `FolioSVN-v1.0.0.dmg`，文件名不带架构后缀（应用仍为 arm64）。DMG 由 Homebrew 提供的 `create-dmg` 生成，提供固定窗口尺寸、大图标、应用与 Applications 左右拖拽布局，并在可用时使用 AppIcon 作为卷图标。DMG 内含已签名、随最外层 DMG 一同接受公证检查的 `Folio SVN.app`。DMG 本身签名、公证并 staple，发布前会完成最终挂载验证。
