...
服务器切换先清除旧会话的路径、条目和导航历史；连接失败或取消后不保留可写目标。连接、目录读取和搜索使用会话代次与请求 ID 校验异步结果，只有最新请求可更新界面或回滚状态；后退/前进栈只在导航成功后更新。侧边栏发起新连接会取消上一连接任务。
...
| 打开文件 | export 到只读快照，再复制可编辑副本并 `NSWorkspace.open` | 外部应用修改不影响版本快照 |
...
- 目录缓存有效期为 25 分钟（1500 秒）。未过期时直接使用，不请求服务端；过期时先立即展示缓存，再后台异步刷新页面、展开目录或侧边栏目录。后台失败保留缓存，不阻塞浏览；页面刷新失败时提示手动刷新。
- 同一仓库、同一 URL 的后台刷新合并为一个请求。切换页面或服务器后，旧结果不得覆盖新页面；手动刷新和写入导致的缓存失效优先于后台刷新。
- 用户点击刷新按钮时，强制读取当前目录及当前树中可见、已展开的子目录，不受有效期限制。按父目录到子目录顺序更新并复用已有节点，保留展开和仍存在的选中项；未展开分支，以及折叠祖先下记忆的展开节点，不请求服务器。单个分支失败时保留原内容，继续其他分支并汇总提示；被用户收起或被父目录刷新删除的节点跳过，导航后旧刷新停止更新界面。
- TTL 在读取缓存（进入目录或重新展开文件夹）时检查，不是持续轮询定时器。
- 当前目录的递归搜索索引建立成功时，将同一份 `svn list -R` 结果按父目录拆分为单层快照，并在同一数据库事务中更新搜索索引和该子树的目录缓存；空目录写入空快照，已从递归结果消失的旧目录缓存同时删除。复用方向仅为“完整递归索引 → 目录缓存”，部分目录缓存不能反向视为完整索引。
- 写操作成功后，使父目录、目标路径和受影响的收藏记录失效。
- 缓存必须记录获取时间和对应 revision。
- SVN 返回提交成功后，收藏同步、缓存清理或目录刷新失败仅作为警告，不得把成功提交报告为失败或自动重试。目录刷新失败时移除过期条目，保留成功 revision，提示手动刷新。被后续导航取代的提交后刷新不得清空新目录。

### 8.2 打开文件缓存

缓存键：`repositoryID + SHA256(完整源 URL) + revision`，使用 `RevisionSnapshots-v1` 命名空间，排除可能已被用户编辑的旧 `OpenCache`。不同主机、路径和 revision 使用不同文件。

版本快照权限设为只读，历史比较直接读取快照。每次交给默认应用打开时，复制到 `OpenDocuments/<UUID>/文件名` 并赋予写权限；这些副本不会作为历史快照复用。

下载先导出到目标目录中的唯一临时路径。未授权覆盖时，通过 macOS `renameatx_np(RENAME_EXCL)` 原子发布文件或目录；即使下载途中 Finder 创建了同名目标，也必须拒绝覆盖并清理临时内容。菜单下载和 Finder 文件承诺都过滤同时选中的父目录后代，避免重复递归下载。
...
- 搜索固定为当前目录及其子目录，索引按需自动建立；工具栏不提供单独的索引刷新或范围选择控件。
- 本地缓存默认上限和企业集中策略。
